...
import com.example.androidtechnote.databinding.ActivityKhCed7Binding
...
class KH_CED7_Activity : AppCompatActivity() {

    lateinit var binding: ActivityKhCed7Binding

    private var deviceAddress: String = ""
    private var bleService: BLEService_KH_CED7? = null
    ...
    private val serviceConnection = object : ServiceConnection {
        ...
        override fun onServiceConnected(name: ComponentName?, service: IBinder?) {
            bleService = (service as BLEService_KH_CED7.LocalBinder).service
        ...
        override fun onReceive(context: Context?, intent: Intent?) {
            when (intent?.action) {
                ...
                BLEService_F400.ACTION_GATT_DISCONNECTED -> {
                    ...
                    Toast.makeText(this@KH_CED7_Activity, "BLE: 연결 해제", Toast.LENGTH_SHORT).show()
                }
    ...
    private fun writeWatchCharacteristic(writeValue: ByteArray){
        writeWatchChar?.let {  writeWatch ->
            writeWatch.apply {
                ...
                bleService?.bluetoothGatt?.writeCharacteristic(this)
            }
        }
    }
    private fun setDescriptor(char: BluetoothGattCharacteristic, indicateEnabled: Boolean){
        bleService?.bluetoothGatt?.setCharacteristicNotification(char, true)
        ...
        bleService?.bluetoothGatt?.writeDescriptor(descriptor)
    }
